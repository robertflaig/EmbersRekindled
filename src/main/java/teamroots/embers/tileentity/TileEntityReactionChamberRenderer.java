...
public class TileEntityReactionChamberRenderer extends TileEntitySpecialRenderer<TileEntityReactionChamber> {
	public ResourceLocation texture = new ResourceLocation(Embers.MODID + ":textures/blocks/pipe_tex.png");

	public TileEntityReactionChamberRenderer(){
		super();
	}
	
	@Override
	public void render(TileEntityReactionChamber tile, double x, double y, double z, float partialTicks, int destroyStage, float tileAlpha){
		if (tile != null){
	...
	private boolean shouldRenderLip(TileEntityReactionChamber pipe, Direction facing) {
	...
	private boolean shouldRenderPipe(TileEntityReactionChamber pipe, Direction facing) {
		EnumPipeConnection connection = getPipeConnection(pipe.getWorld(), pipe.getPos(), facing);
